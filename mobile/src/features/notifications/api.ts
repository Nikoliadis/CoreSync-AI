 ...
 *
 * `no-unnecessary-type-assertion` is switched off for this file in the ESLint config,
 * and the reason is written there: the route union is generated by `expo start` into
 * `.expo/types`, which CI cannot produce, so the assertion is required locally and looks
 * redundant in CI.
 *
 * It lives here, once, behind the `startsWith("/")` check in `routeFor` — rather than at
 * each call site, where the reasoning would be repeated and eventually copied somewhere
 * it does not hold.
 */
